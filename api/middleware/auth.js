...
module.exports = function *auth(next){
  ...
  this.assert(typeof login.name === 'string','name is not a string',400);
  this.assert(typeof login.pass === 'string','pass is not a string',400);
  const user = yield models.User.findOne(null, '+password')
  .where(username, login.name)
  .exec();
  this.assert(user, 401, username+' not found');
